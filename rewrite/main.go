...
// dur is the duration of time spent answering questions.
...
func main() {
	...
	for _, o := range *flagOps {
		...
		if strings.Count(strings.Join(opsUni, ""), op) > 0 {
			...
			*flagUni = true // Use unicode ops if these are provided in the arg.
			continue
		}
	...
	// Populate the penalised map with allowed ops.
	...
	for {
		...
		for i := 0; i < quests; i++ {
			if err := ask(operands, maxRange); err != nil {
